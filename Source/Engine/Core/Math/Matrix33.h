...
namespace kiko {
	...
	public:
		...
		Matrix33() = default;
		Matrix33(const vec3& row1, const vec3& row2, const vec3& row3) { rows[0] = row1; rows[1] = row2; rows[2] = row3; };
	...
	inline Matrix33 Matrix33::CreateIdentity() { return { {1, 0, 0}, {0, 1, 0}, {0, 0, 1 } }; }
	...
	inline Matrix33 Matrix33::CreateRotation(float radians) {
		...
		return { {c, -s, 0.0f}, {s, c, 0.0f}, {0, 0, 1} };
	}

	inline vec2 Matrix33::GetTranslation() const { return { rows[0][2], rows[1][2] }; }
	inline float Matrix33::GetRotation() const { return std::atan2(rows[1][0], rows[0][0]); }
